...
clf_path = './data/xgb.pickle_new.dat'
with open(clf_path, "rb") as f:
    clf = pickle.load(f)
model = word2vec.Word2Vec.load('./data/model_300dim.pkl')

def activity(smiles):
    vals = [jak2(s) if verify_sequence(s) else 0 for s in smiles]
    return vals
...
def jak2(smile):
    ...
    fp=[DfVec(x).vec.tolist() for x in
           sentences2vec(np.array([sentence]), model, unseen='UNK')]
...
def metrics_loading():
    ...
    loadings['activity'] = lambda *args: None
